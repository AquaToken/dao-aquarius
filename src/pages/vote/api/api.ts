...
import { getBribesUrl, getMarketKeysUrl, getRewardsUrl, getVotingTrackerUrl } from 'helpers/url';
...
const getRewardsApiUrl = () =>
    `${getRewardsUrl()}?ordering=-daily_total_reward&page=1&page_size=200`;
...
const getPairUrl = (sortType: SortTypes, pageSize: number, page: number): string => {
    switch (sortType) {
        case SortTypes.popular:
            return `${getVotingTrackerUrl()}top-voted/?limit=${pageSize}&page=${page}`;
        case SortTypes.topVoted:
            return `${getVotingTrackerUrl()}top-volume/?limit=${pageSize}&page=${page}`;
    }
};
...
const addKeysToMarketVotes = async (votes: MarketVotes[], count) => {
    ...
    const [marketsKeys, bribes] = await Promise.all([
        ...
        axios.get<ListResponse<MarketBribes>>(`${getBribesUrl()}bribes/?limit=200`, {
            params: bribesParams,
        }),
    ]);
...
export const updateVotesForMarketKeys = async (pairs: PairStats[]): Promise<PairStats[]> => {
    ...
    const marketsVotes = await axios.get<ListResponse<MarketVotes>>(getVotingTrackerUrl(), {
        params,
    });
...
export const getUserPairsList = async (keys: string[]) => {
    ...
    const [marketsVotes, bribes] = await Promise.all([
        axios.get<ListResponse<MarketVotes>>(getVotingTrackerUrl(), {
...
export const getPairsWithBribes = async (pageSize: number, page: number) => {
    const bribes = await axios.get<ListResponse<MarketBribes>>(
        `${getBribesUrl()}bribes/?limit=${pageSize}&page=${page}`,
    );
...
export const getTotalVotingStats = (): Promise<TotalStats> =>
    axios.get<TotalStats>(`${getVotingTrackerUrl()}stats/`).then(({ data }) => data);

export const getUpcomingBribesForMarket = (marketKey: string): Promise<UpcomingBribe[]> =>
    axios
        .get<
            ListResponse<UpcomingBribe>
        >(`${getBribesUrl()}pending-bribes/?limit=200&ordering=start_at&market_key=${marketKey}`)
        .then(({ data }) => data.results);

export const getRewards = (): Promise<Rewards[]> =>
    axios.get<ListResponse<Rewards>>(getRewardsApiUrl()).then(res => res.data.results);
